<div>
        ...
        <div class="flex flex-row justify-between">
            ...
            <div class="flex flex-col gap-2 md:flex-row justify-start text-sm lg:text-base">
                ...
                <div class="px-2 md:px-0 inline-flex w-1/6  h-12 md:mx-3 mb-2 bg-transparent">
                    <select wire:change='updateStateSprint({{ $idSprint }}, $event.target.value)' name="state" id="state" class="inputs">
                    ...
                    <div class="flex justify-center">
                        ...
                        >
                            ...
                            <div x-ref="panel" x-show="open" x-on:click.outside="close($refs.button)" :id="$id('dropdown-button')" style="display: none;" class="absolute right-10 top-3 mt-2 w-32 rounded-md bg-gray-200 z-10">
                                <!-- Botón Nuevo -->
                                <div wire:click="modalCreateSprint()" class="flex px-4 py-2 text-sm text-black cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-plus mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                        <path d="M12 5l0 14" />
                                        <path d="M5 12l14 0" />
                                    ...
                                    Nuevo</div>
                                <!-- Botón Editar -->
                                <div wire:click="showEditSprint({{ $idSprint }})" class="@if($firstSprint->state == 'Cerrado') hidden  @endif flex px-4 py-2 text-sm text-black cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-edit mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                    Editar</div>
                                <!-- Botón Eliminar -->
                                <div wire:click="$emit('deleteSprint',{{ $idSprint }})" class="@if($firstSprint->state == 'Cerrado' || $firstSprint->state == 'Curso') hidden @endif flex px-4 py-2 text-sm text-red-600 cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-trash mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                        <path d="M4 7l16 0" />
                                        <path d="M10 11l0 6" />
                                        <path d="M14 11l0 6" />
    ...
    <div class="sm:rounded-lg px-4 py-4 @if($sprints->isEmpty()) hidden @endif">
        ...
        <div class="flex flex-col gap-2 md:flex-row justify-between text-sm lg:text-base">
            ...
            <div class=" md:inline-flex w-1/6 h-12 bg-transparent mb-2">
                <button wire:click="modalCreateActivity()" @if($firstSprint->state == 'Pendiente' && Auth::user()->type_user != 1 && Auth::user()->area_id != 1)disabled class="btnDisabled"@else class="btnNuevo"@endif>
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-plus mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                        <path d="M12 5l0 14" />
                        <path d="M5 12l14 0" />
        ...
        <div class="tableStyle">
            <table class="w-full whitespace-no-wrap table table-hover">
                ...
                <tbody>
                    ...
                    <tr class="trTable">
                        <td class="px-4 py-2">
                            <div wire:click="showActivity({{ $activity->id }})" class="flex flex-col items-center text-center cursor-pointer">
                                <p class="text-xs mb-2 text-justify">{{ $activity->tittle }}</p>
                                @if (!empty($activity->image))
                                <div class="relative h-22 w-full px-3 pt-2">
                                    @if ($activity->image == true)
                                    <img src="{{ asset('activities/' . $activity->image) }}" alt="Report Image" class="h-20 w-32 object-cover mx-auto">
                                    @endif
                                </div>
                                @endif
                        ...
                        <td class="px-4 py-2">
                            <div class="w-56 mx-auto text-justify ">
                                <p class="@if($activity->state == 'Resuelto') font-semibold @else hidden @endif">{{ $activity->delegate->name }} {{ $activity->delegate->lastname }}</p>
                                <select wire:change.defer='updateDelegate({{ $activity->id }}, $event.target.value)'  name="delegate" id="delegate" class="w-full text-sm inpSelectTable w-min-full @if($activity->state == 'Resuelto') hidden @endif" @if($firstSprint->state == 'Pendiente' && Auth::user()->type_user != 1 && Auth::user()->area_id != 1)disabled @endif>
                                    <option selected value={{ $activity->delegate->id }}>{{ $activity->delegate->name }} </option>
                        ...
                        <td class="px-4 py-2">
                            <select wire:change='updateState({{ $activity->id }}, $event.target.value)' name="state" id="state" 
                        ...
                        <td class="px-4 py-2 text-justify">
                            <div class="w-36 mx-auto">
                                <span class="font-semibold inline-block">Para:</span> {{ \Carbon\Carbon::parse($activity->expected_date)->locale('es')->isoFormat('D[-]MMMM[-]YYYY') }}<br>
                                <span class="font-semibold inline-block">Se delegó:</span> {{ \Carbon\Carbon::parse($activity->delegated_date)->locale('es')->isoFormat('D[-]MMMM[-]YYYY') }}
                            </div>
                        </td>
                        <td class="px-4 py-2">
                            <div class="w-40 mx-auto text-justify">
                                <span class=" font-semibold "> {{ $activity->user->name }} {{ $activity->user->lastname }}  </span> <br>
                                <span class=" font-mono"> {{ \Carbon\Carbon::parse($activity->created_at)->locale('es')->isoFormat('D[-]MMMM[-]YYYY') }} </span><br> 
                                <span  class="italic"> {{ $activity->created_at->diffForHumans(null, false, false, 1) }}  </span> 
                            </div>
                        </td>
                        <td class="px-4 py-5">
                            <div class="flex justify-center">
                                ...
                                x-on:keydown.escape.prevent.stop="close($refs.button)" x-id="['dropdown-button']" class="relative">
                                <button x-ref="button" x-on:click="toggle()" :aria-expanded="open" :aria-controls="$id('dropdown-button')" type="button" class="flex items-center px-5 py-2.5" @if($firstSprint->state == 'Pendiente' && Auth::user()->type_user != 1 && Auth::user()->area_id != 1)disabled @endif>
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-dots-vertical" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                        <path d="M12 12m-1 0a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" />
                                        <path d="M12 19m-1 0a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" />
                                ...
                                <div x-ref="panel" x-show="open" x-on:click.outside="close($refs.button)" :id="$id('dropdown-button')" style="display: none;" class="absolute right-10 top-3 mt-2 w-32 rounded-md bg-gray-200 z-10">
                                    <!-- Botón Editar -->
                                    <div wire:click="showEditActivity({{ $activity->id }})" class="@if($activity->state == 'Resuelto') hidden @endif flex content-center px-4 py-2 text-sm text-black cursor-pointer">
                                        ...
                                        Editar</div>
                                    <!-- Botón Reincidencia -->
                                    <div wire:click="$emit('deleteActivity',{{ $activity->id }})" class="@if($activity->state != 'Abierto') hidden @endif flex content-center px-4 py-2 text-sm text-red-600 cursor-pointer">
        ...
        <div class="flex text:md justify-center h-screen items-center top-0 left-0 z-40 w-full fixed">
            <div class="flex flex-col md:w-2/5 mx-auto rounded-lg   overflow-y-auto " style="max-height: 90%;">
                <div class="flex flex-row justify-between px-6 py-4 bg-gray-100 text-white rounded-tl-lg rounded-tr-lg">
                    <h3 class="text-xl text-secundaryColor font-medium title-font  w-full border-l-4 border-secundaryColor pl-4 py-2">Backlog</h3>
                    <svg wire:click="modalBacklog()" wire:loading.remove wire:target="modalBacklog()" class="w-6 h-6 my-2 cursor-pointer text-black hover:stroke-2" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                ...
                <div class="modalFooter">
                    <button class="btnSave" wire:click="modalBacklog()" wire:loading.remove wire:target="modalBacklog()"> Cerrar </button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="top-20 left-0 z-50 max-h-full overflow-y-auto @if($modalCreateSprint) block  @else hidden @endif">
        ...
        <div class="flex text:md justify-center h-screen items-center top-0 left-0 z-40 w-full fixed">
            <div class="flex flex-col md:w-2/5 mx-auto rounded-lg   overflow-y-auto " style="max-height: 90%;">
                <div class="flex flex-row justify-between px-6 py-4 bg-gray-100 text-white rounded-tl-lg rounded-tr-lg">
                    @if($showUpdateSprint)
                    <h3 class="text-xl text-secundaryColor font-medium title-font  w-full border-l-4 border-secundaryColor pl-4 py-2">Editar sprint {{ $sprintEdit->number }}</h3>
                    @else
                    <h3 class="text-xl text-secundaryColor font-medium title-font  w-full border-l-4 border-secundaryColor pl-4 py-2">Crear sprint</h3>
                    @endif
                    <svg wire:click="modalCreateSprint" wire:loading.remove wire:target="modalCreateSprint" class="w-6 h-6 my-2 cursor-pointer text-black hover:stroke-2" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M18 6l-12 12"></path>
                        <path d="M6 6l12 12"></path>
                    </svg>
                    ...
                    <div class="w-full md-3/4 mb-5 mt-5 flex flex-col">
                        <div class="-mx-3 mb-6">
                            <div class="w-full flex flex-col px-3 mb-6">
                                ...
                                <input wire:model='name_sprint' required type="text" placeholder="Nombre" name="name_sprint" id="name_sprint" class="inputs">
                                <div>
                        ...
                        <div class="-mx-3 flex flex-row">
                            <div class="w-full flex flex-col px-3 mb-6">
                                ...
                                <input wire:model='start_date' required type="date" name="start_date" id="start_date" class="inputs">
                                <div>
                                    <span class="text-red-600 text-xs italic">
                                        @error('start_date')
                                        <span class="pl-2 text-red-600 text-xs italic">
                            ...
                            <div class="w-full flex flex-col px-3">
                                ...
                                <input wire:model='end_date' required type="date" name="end_date" id="end_date" class="inputs">
                                <div>
                                    <span class="text-red-600 text-xs italic">
                                        @error('end_date')
                                        <span class="pl-2 text-red-600 text-xs italic">
                ...
                <div class="modalFooter">
                    @if($showUpdateSprint)
                    <button class="btnSave" wire:click="updateSprint({{ $sprintEdit->id }})" wire:loading.remove wire:target="updateSprint({{ $sprintEdit->id }})"> Guardar </button>
                    @else
        ...
        <div class="flex text:md justify-center h-full items-center top-0 left-0 z-40 w-full fixed px-2 smd:px-0">
            <div class="flex flex-col  md:w-3/4 mx-auto rounded-lg   overflow-y-auto " style="max-height: 90%;">
                ...
                <div class="flex flex-row justify-between px-6 py-4 bg-gray-100 text-white rounded-tl-lg rounded-tr-lg">
                    ...
                    <svg wire:click="modalShowActivity"  wire:target="modalShowActivity" class="w-6 h-6 cursor-pointer text-black hover:stroke-2" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M18 6l-12 12"></path>
                        <path d="M6 6l12 12"></path>
                    </svg>
                ...
                <div class="flex flex-row justify-end px-6 py-4 bg-main-fund text-white rounded-tl-lg rounded-tr-lg">
                    <svg wire:click="modalShowActivity" wire:loading.remove wire:target="modalShowActivity" class="w-6 h-6 cursor-pointer text-black hover:stroke-2" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M18 6l-12 12"></path>
                        <path d="M6 6l12 12"></path>
                    </svg>
                ...
                <div class="flex flex-col lg:flex-row items-stretch   px-6 py-2 bg-white overflow-y-auto text-sm">
                    <div class="w-full lg:w-1/2 md-3/4 mb-5 md:mb-0 mt-3 flex flex-col justify-between px-5 lg:border-r-2 border-gray-400">
                        ...
                        <div class="w-auto flex flex-row  my-6">
                            ...
                            <button class="btnSave"  style="border-radius: 0rem 0.5rem 0.5rem 0rem !important" wire:click="updateChat({{ $activityShow->id }})">
                ...
                <div class="photos w-full lg:w-1/2 px-5 ">
                    ...
                    <div class="flex justify-center items-center">
                        <a href="{{ asset('activities/' . $activityShow->image) }}" download="{{ basename($activityShow->image) }}" class="btnSecondary" style="color: white;">
        ...
        <div class="flex text:md justify-center h-screen items-center top-0 left-0 z-40 w-full fixed">
            <div class="flex flex-col md:w-2/5 mx-auto rounded-lg   overflow-y-auto " style="max-height: 90%;">
                <div class="flex flex-row justify-between px-6 py-4 bg-gray-100 text-white rounded-tl-lg rounded-tr-lg">
                    ...
                    <h3 class="text-xl text-secundaryColor font-medium title-font  w-full border-l-4 border-secundaryColor pl-4 py-2">Editar actividad</h3>
                    @else
                    <h3 class="text-xl text-secundaryColor font-medium title-font  w-full border-l-4 border-secundaryColor pl-4 py-2">Crear actividad</h3>
                    @endif
                    <svg wire:click="modalCreateActivity()" wire:loading.remove wire:target="modalCreateActivity" class="w-6 h-6 my-2 cursor-pointer text-black hover:stroke-2" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M18 6l-12 12"></path>
                        <path d="M6 6l12 12"></path>
                    </svg>
                    ...
                    <div class="w-full md-3/4 mb-5 mt-5 flex flex-col">
                        <div class="-mx-3 mb-6 flex flex-row">
                            <div class="w-full flex flex-col px-3 mb-6">
                                ...
                                <input wire:model='tittle' required type="text" placeholder="Título" name="tittle" id="tittle" class="inputs">
                                <div>
                                    <span class="text-red-600 text-xs italic">
                                        @error('tittle')
                                        <span class="pl-2 text-red-600 text-xs italic">
                        ...
                        <div class="-mx-3 mb-6">
                            <div class="w-full flex flex-col px-3 mb-6">
                                ...
                                <textarea wire:model='description' type="text" rows="10" placeholder="Describa la observación y especifique el objetivo a cumplir." name="description" id="description" class="inputs"></textarea>                                                                                                                                                                                       
                                <div>
                                    <span class="text-red-600 text-xs italic">
                                        @error('description')
                                        <span class="pl-2 text-red-600 text-xs italic">
                        ...
                        <div class="-mx-3 mb-6 flex flex-row">
                            ...
                            <div class="w-full flex flex-col px-3 mb-6">
                                ...
                                <input wire:model='expected_date' required type="date" name="expected_date" id="expected_date" class="inputs">
                                <div>
                                    <span class="text-red-600 text-xs italic">
                                        @error('expected_date')
                                        <span class="pl-2 text-red-600 text-xs italic">
                            ...
                            <div class="w-full flex flex-col px-3">
                                ...
                                <div class="flex justify-center gap-20">
                                    <div class="flex flex-col items-center">
                                        <input type="checkbox" wire:model="priority1" wire:change="selectPriority($event.target.value)" value="Alto" class="priority-checkbox border-red-600 bg-red-600" style="height: 24px; width: 24px; accent-color: #dd4231;" />
                                        <label for="priority1" class="mt-2">Alto</label>
                                    </div>
                                    <div class="flex flex-col items-center">
                                        <input type="checkbox" wire:model="priority2" wire:change="selectPriority($event.target.value)" value="Medio" class="priority-checkbox border-yellow-400 bg-yellow-400" style="height: 24px; width: 24px; accent-color: #f6c03e;" />
                                        <label for="priority2" class="mt-2">Medio</label>
                                    </div>
                                    <div class="flex flex-col items-center">
                                        <input type="checkbox" wire:model="priority3" wire:change="selectPriority($event.target.value)" value="Bajo" class="priority-checkbox border-secondary bg-secondary" style="height: 24px; width: 24px; accent-color: #0062cc;" />
                ...
                <div class="modalFooter">
                    @if($showUpdateActivity)
                    <button class="btnSave" wire:click="updateActivity({{ $activityEdit->id }})" wire:loading.remove wire:target="updateActivity({{ $activityEdit->id }})"> Guardar </button>
                    @else
                    <button class="btnSave" wire:click="createActivity()" wire:loading.remove wire:target="createActivity">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-device-floppy mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                            <path d="M6 4h10l4 4v10a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2" />
                            <path d="M12 14m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" />
