            ...
            <div class="mb-2 inline-flex h-10 w-2/6 bg-transparent px-2 md:mx-3 md:px-0">
                <label class="inputs" wire:ignore>
                    <div class="flex inline-block">
                        ...
                        <span class="text-base" id="spanDate">{{ $startDate }} - {{ $endDate }}</span>
                    </div>
                </label>
            </div>
            <div class="mb-2 inline-flex h-12 w-2/6 bg-transparent px-2 md:mx-3 md:px-0">
                Qty días. {{ $days_filtred }}
            </div>
        </div>
    ...
    @push('js')
        <script>
            ...
            document.addEventListener('livewire:load', function() {
                ...
                var end = endOfMonth.format('YYYY-MM-DD');
                ...
                // Actualizar el span al seleccionar un nuevo rango de fechas
                $('#daterange').on('apply.daterangepicker', function(ev, picker) {
                    $('#spanDate').html(picker.startDate.format('YYYY-MM-DD') + ' - ' + picker.endDate.format('YYYY-MM-DD'));
                    Livewire.emit('setDate', picker.startDate.format('YYYY-MM-DD') + ' - ' + picker.endDate.format('YYYY-MM-DD'));
                });
            });
            // GRAFICA
            document.addEventListener('livewire:load', function() {
                var options = {
                                ...
                                total: {
                                    ...
                                    style: {
                                        ...
                                        color: function({
                                            ...
                                            return seriesIndex === 4 ? '#000' :
                                            '#fff'; // Color de texto negro para 'Asignar', blanco para otros
                                        }
                                    }
                                }
                            }
                    ...
                    dataLabels: {
                        enabled: true,
                        style: {
                            colors: ['#fff', '#fff', '#fff', '#fff', '#000'] // Colores del texto para cada serie
                        }
                    },
                    stroke: {
                        width: 1,
